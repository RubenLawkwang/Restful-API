...
include_once(__DIR__ . '/../models/category.php'); // include category configuration
...
switch ($requestMethod) {
    ...
    case 'GET': // Handle read one and read all request
        if (!empty($_GET["cat_id"])) {
            $response = $category->readOne($_GET["cat_id"]);
        } else {
            $response = $category->read();
        }
        break;

    case 'POST': // Handle create request
        var_dump($data);
        $response = $category->create($data->cat_name, $data->cat_id);
        if ($response) {
    ...
    case 'PUT': // Handle update request
        $response = $category->update($data->cat_name, $data->cat_id);
        if ($response) {
            http_response_code(200);
    ...
    case 'DELETE': // Handle delete request
        $response = $category->delete($data->cat_id);
        if ($response) {
            http_response_code(200);
            echo json_encode(array('message' => 'Category deleted successfully'));
        } else {
            http_response_code(500); // error 500 Internal Server Error
            echo json_encode(array('message' => 'Category deletion failed'));
        }
        break;
    default:
